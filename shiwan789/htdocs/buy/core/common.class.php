...
class common
{
    ...
    public function get_randlist($params, $no_cache = 0)
    {
		...
		$sql1 = "select * from ". TB_PREFIX . $tbname . "_list order by create_time desc limit 50";
		
        //$sql1 = "select * from " . TB_PREFIX . "cate_relation as a $user_index left join " . TB_PREFIX . $tbname . "_list as b on a.id=b." . $tbname . "_id where cate_id='$cate_id' limit 50";
		...
		$result1 = $this->dbm->query($sql1);
		//var_dump($result1['list']);die;
		$rang_count = count($result1['list']);
		
		if($rang_count == 0){
			$rang_count = 6;
		}else{
			$rang_count = $rang_count -1;
		}
		
		for($i=0;$i < $count; $i++)
		{
			$rangs = mt_rand(0,$rang_count);
			
			if(count($result1['list'])>0)
				$result['list'][$i]=$result1['list'][$rangs];
		}
