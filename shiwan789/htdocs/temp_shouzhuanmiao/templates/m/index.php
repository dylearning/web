	...
	<div class="box wrap">
		...
		<div class="box wrap-content">
			<div class="box app-area">
				...
				<?php
				$a = $c->get_recommend(array('area_id'=>0,'pagesize'=>2,'count'=>0,'rewrite'=>SITE_REWRITEURL,'where'=>'area_type=0','order'=>'order by area_order asc'));//print_r($a);//推荐位?>
					<?php if(!empty($a['list'])){foreach($a['list'] as $k=> $v){?>
					<?php
					$b = $c->get_list(array('count'=>0,'pagesize'=>15,'rewrite'=>SITE_REWRITEURL,'where'=>'app_id in ('.$v['id_list'].')','order'=>'order by app_order desc,app_id asc'));//print_r($b);//推荐位的详细?>
					<p class="line-t-10"></p>
